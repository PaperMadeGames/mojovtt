...
export default class ItemGrantAdvancement extends Advancement {
  ...
  static get metadata() {
    return foundry.utils.mergeObject(super.metadata, {
      ...
      order: 40,
      icon: "systems/mojo/icons/svg/item-grant.svg",
      title: game.i18n.localize("MOJO.AdvancementItemGrantTitle"),
      hint: game.i18n.localize("MOJO.AdvancementItemGrantHint"),
      apps: {
  ...
  summaryForLevel(level, { configMode=false }={}) {
    ...
    if ( !this.value.added || configMode ) {
      return this.configuration.items.reduce((html, uuid) => html + mojo.utils.linkForUuid(uuid), "");
    }
    ...
    for ( const [uuid, selected] of Object.entries(data) ) {
      ...
      if ( !itemData ) {
        ...
        itemData = source.clone({
          ...
          "flags.mojo.sourceId": uuid,
          "flags.mojo.advancementOrigin": `${this.item.id}.${this.id}`
  ...
  restore(level, data) {
    ...
    for ( const item of data.items ) {
      ...
      updates[item._id] = item.flags.mojo.sourceId;
    }
    this.updateSource({"value.added": updates});
  }
