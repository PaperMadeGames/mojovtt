 ...
 * @param {string} trait  Trait as defined in `CONFIG.MOJO.traits`.
 * @returns {object}      Trait categories defined within `CONFIG.MOJO`.
 */
export function categories(trait) {
  const traitConfig = CONFIG.MOJO.traits[trait];
  return CONFIG.MOJO[traitConfig.configKey ?? trait];
}
...
export async function choices(trait, chosen=new Set()) {
  ...
  if ( traitConfig.children ) {
    for ( const [categoryKey, childrenKey] of Object.entries(traitConfig.children) ) {
      const children = CONFIG.MOJO[childrenKey];
  ...
  if ( traitConfig.subtypes ) {
    ...
    const map = CONFIG.MOJO[`${trait}ProficienciesMap`];

    // Merge all IDs lists together
    const ids = traitConfig.subtypes.ids.reduce((obj, key) => {
      if ( CONFIG.MOJO[key] ) Object.assign(obj, CONFIG.MOJO[key]);
      return obj;
    }, {});
  ...
  // Sort Categories
  if ( traitConfig.sortCategories ) data = mojo.utils.sortObjectEntries(data, "label");

  // Sort Children
  for ( const category of Object.values(data) ) {
    if ( !category.children ) continue;
    category.children = mojo.utils.sortObjectEntries(category.children, "label");
  }
...
export function getBaseItem(identifier, { indexOnly=false, fullItem=false }={}) {
  let pack = CONFIG.MOJO.sourcePacks.ITEMS;
...
export function traitIndexFields() {
  const fields = [];
  for ( const traitConfig of Object.values(CONFIG.MOJO.traits) ) {
...
export function traitLabel(trait, count) {
  ...
  return game.i18n.localize(`MOJO.Trait${typeCap}Plural.${pluralRule}`);
}
...
export function keyLabel(trait, key) {
  ...
  for ( const childrenKey of Object.values(traitConfig.children ?? {}) ) {
    if ( CONFIG.MOJO[childrenKey]?.[key] ) return CONFIG.MOJO[childrenKey]?.[key];
  }

  for ( const idsKey of traitConfig.subtypes?.ids ?? [] ) {
    if ( !CONFIG.MOJO[idsKey]?.[key] ) continue;
    const index = getBaseItem(CONFIG.MOJO[idsKey][key], { indexOnly: true });
...
export function choiceLabel(trait, choice) {
  ...
  if ( !choice.pool ) {
    return game.i18n.format("MOJO.TraitConfigChooseAny", {
      count: choice.count,
  ...
  return game.i18n.format("MOJO.TraitConfigChooseList", {
    count: choice.count,
