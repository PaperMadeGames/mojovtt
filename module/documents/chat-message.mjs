...
export function highlightCriticalSuccessFailure(message, html, data) {
  ...
  if ( !d20Roll ) return;
  d20Roll = mojo.dice.D20Roll.fromRoll(d20Roll);
...
export function displayChatActionButtons(message, html, data) {
  const chatCard = html.find(".mojo.chat-card");
...
export function addChatMessageContextOptions(html, options) {
  ...
  options.push(
    {
      name: game.i18n.localize("MOJO.ChatContextDamage"),
      icon: '<i class="fas fa-user-minus"></i>',
      condition: canApply,
      callback: li => applyChatCardDamage(li, 1)
    },
    {
      name: game.i18n.localize("MOJO.ChatContextHealing"),
      icon: '<i class="fas fa-user-plus"></i>',
      condition: canApply,
      callback: li => applyChatCardDamage(li, -1)
    },
    {
      name: game.i18n.localize("MOJO.ChatContextTempHP"),
      icon: '<i class="fas fa-user-clock"></i>',
      condition: canApply,
      callback: li => applyChatCardTemp(li)
    },
    {
      name: game.i18n.localize("MOJO.ChatContextDoubleDamage"),
      icon: '<i class="fas fa-user-injured"></i>',
      condition: canApply,
      callback: li => applyChatCardDamage(li, 2)
    },
    {
      name: game.i18n.localize("MOJO.ChatContextHalfDamage"),
...
export function onRenderChatMessage(app, html, data) {
  ...
  if (game.settings.get("mojo", "autoCollapseItemCards")) html.find(".card-content").hide();
}
