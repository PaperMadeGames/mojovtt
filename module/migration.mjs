...
export const migrateWorld = async function() {
  ...
  for ( const [actor, valid] of actors ) {
    try {
      ...
      err.message = `Failed mojo system migration for Actor ${actor.name}: ${err.message}`;
      console.error(err);
    }
  }
  ...
  for ( const [item, valid] of items ) {
    try {
      ...
      err.message = `Failed mojo system migration for Item ${item.name}: ${err.message}`;
      console.error(err);
    }
  }
  ...
  for ( const m of game.macros ) {
    try {
      ...
      err.message = `Failed mojo system migration for Macro ${m.name}: ${err.message}`;
      console.error(err);
    }
  }
  ...
  for ( let s of game.scenes ) {
    try {
      ...
      err.message = `Failed mojo system migration for Scene ${s.name}: ${err.message}`;
      console.error(err);
    }
  }
  ...
  game.settings.set("mojo", "systemMigrationVersion", game.system.version);
...
export const migrateCompendium = async function(pack) {
  ...
  for ( let doc of documents ) {
    ...
    // Handle migration failures
      ...
      err.message = `Failed mojo system migration for document ${doc.name} in pack ${pack.collection}: ${err.message}`;
      console.error(err);
    }
  }
...
export async function refreshCompendium(pack) {
  ...
  mojo.moduleArt.suppressArt = true;
  ...
  await pack.configure({locked: wasLocked});
  mojo.moduleArt.suppressArt = false;
...
export const migrateArmorClass = async function(pack) {
  ...
  const updates = [];
  const armor = new Set(Object.keys(CONFIG.MOJO.armorTypes));
...
export const getMigrationData = async function() {
  const data = {};
  try {
    const icons = await fetch("systems/mojo/json/icon-migration.json");
    const spellIcons = await fetch("systems/mojo/json/spell-icon-migration.json");
...
function _migrateTokenImage(actorData, updateData) {
  const oldSystemPNG = /^systems\/mojo\/tokens\/([a-z]+)\/([A-z]+).png$/;
  for ( const path of ["texture.src", "prototypeToken.texture.src"] ) {
    ...
    if ( oldSystemPNG.test(v) ) {
      ...
      updateData[path] = `systems/mojo/tokens/${type}/${fileName}.webp`;
    }
  }
  return updateData;
}
...
/**
 * Migrate macros from the old 'mojo.rollItemMacro' and 'mojo.macros' commands to the new location.
 * @param {object} macro       Macro data to migrate.
 * @param {object} updateData  Existing update to expand upon.
 * @returns {object}           The updateData to apply.
 */
function _migrateMacroCommands(macro, updateData) {
  if ( macro.command.includes("game.mojo.rollItemMacro") ) {
    updateData.command = macro.command.replaceAll("game.mojo.rollItemMacro", "mojo.documents.macro.rollItem");
  } else if ( macro.command.includes("game.mojo.macros.") ) {
    updateData.command = macro.command.replaceAll("game.mojo.macros.", "mojo.documents.macro.");
  }
  return updateData;
}
...
export async function purgeFlags(pack) {
  const cleanFlags = flags => {
    const flags5e = flags.mojo || null;
    return flags5e ? {mojo: flags5e} : {};
  };
  await pack.configure({locked: false});
