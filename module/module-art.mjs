...
export class ModuleArt {
  ...
  async registerModuleArt() {
    ...
    for ( const module of game.modules ) {
      ...
      try {
        ...
        await this.#parseArtMapping(module.id, mapping, flags["mojo-art-credit"]);
      } catch( e ) {
        console.error(e);
      }
    }
    ...
    // Load system mapping.
    try {
      const mapping = await foundry.utils.fetchJsonWithTimeout("systems/mojo/json/fa-token-mapping.json");
      const credit = `
        <em>
          Token artwork by
  ...
  async #parseArtMapping(moduleId, mapping, credit) {
    let settings = game.settings.get("mojo", "moduleArtConfiguration")?.[moduleId];
...
export class ModuleArtConfig extends FormApplication {
  ...
  constructor(object={}, options={}) {
    object = foundry.utils.mergeObject(game.settings.get("mojo", "moduleArtConfiguration"), object, {inplace: false});
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      title: game.i18n.localize("MOJO.ModuleArtConfigL"),
      id: "module-art-config",
      template: "systems/mojo/templates/apps/module-art-config.html",
      popOut: true,
      width: 600,
      height: "auto"
    });
  ...
  getData(options={}) {
    ...
    context.config.unshift({label: game.system.title, id: game.system.id, ...this.object.mojo});
    return context;
  }
  ...
  async _updateObject(event, formData) {
    await game.settings.set("mojo", "moduleArtConfiguration", foundry.utils.expandObject(formData));
