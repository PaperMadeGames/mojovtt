...
export default class GroupActorSheet extends ActorSheet {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["mojo", "sheet", "actor", "group"],
      template: "systems/mojo/templates/actors/group-sheet.hbs",
  ...
  async getData(options={}) {
    ...
    // Text labels
    context.labels = {
      currencies: Object.entries(CONFIG.MOJO.currencies).reduce((obj, [k, c]) => {
        obj[k] = c.label;
        return obj;
      }, {})
    };
  ...
  #getSummary(stats) {
    ...
    const members = [];
    if ( stats.nMembers ) members.push(`${stats.nMembers} ${game.i18n.localize("MOJO.GroupMembers")}`);
    if ( stats.nVehicles ) members.push(`${stats.nVehicles} ${game.i18n.localize("MOJO.GroupVehicles")}`);
    return game.i18n.format("MOJO.GroupSummary", {members: formatter.format(members)});
  }
  ...
  #prepareMembers() {
    ...
    for ( const member of this.object.system.members ) {
      ...
      m.hp.color = mojo.documents.Actor5e.getHPColor(m.hp.current, m.hp.max).css;
  ...
  #prepareMovementSpeed() {
    ...
    let speeds = [
      [movement.land, `${game.i18n.localize("MOJO.MovementLand")} ${movement.land}`],
      [movement.water, `${game.i18n.localize("MOJO.MovementWater")} ${movement.water}`],
      [movement.air, `${game.i18n.localize("MOJO.MovementAir")} ${movement.air}`]
    ];
  ...
  _createItem(button) {
    ...
    delete system.type;
    const name = game.i18n.format("MOJO.ItemNew", {type: game.i18n.localize(`ITEM.Type${type.capitalize()}`)});
