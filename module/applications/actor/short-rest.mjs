...
export default class ShortRestDialog extends Dialog {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      template: "systems/mojo/templates/apps/short-rest.hbs",
      classes: ["mojo", "dialog"]
    });
  }
  ...
  getData() {
    ...
    // Determine rest type
    const variant = game.settings.get("mojo", "restVariant");
  ...
  static async shortRestDialog({ actor }={}) {
    return new Promise((resolve, reject) => {
      const dlg = new this(actor, {
        title: `${game.i18n.localize("MOJO.ShortRest")}: ${actor.name}`,
        buttons: {
          rest: {
            icon: '<i class="fas fa-bed"></i>',
            label: game.i18n.localize("MOJO.Rest"),
            callback: html => {
              let newDay = false;
              if ( game.settings.get("mojo", "restVariant") !== "epic" ) {
