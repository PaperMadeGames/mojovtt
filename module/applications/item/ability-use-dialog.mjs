...
export default class AbilityUseDialog extends Dialog {
  constructor(item, dialogData={}, options={}) {
    ...
    this.options.classes = ["mojo", "dialog"];
  ...
  static async create(item) {
    ...
    const data = {
      item: item,
      title: game.i18n.format("MOJO.AbilityUseHint", {type: game.i18n.localize(`ITEM.Type${item.type.capitalize()}`), name: item.name}),
    ...
    const html = await renderTemplate("systems/mojo/templates/apps/ability-use.hbs", data);

    // Create the Dialog and return data as a Promise
    const icon = data.isSpell ? "fa-magic" : "fa-fist-raised";
    const label = game.i18n.localize(`MOJO.AbilityUse${data.isSpell ? "Cast" : "Use"}`);
    return new Promise(resolve => {
      const dlg = new this(item, {
        title: `${item.name}: ${game.i18n.localize("MOJO.AbilityUseConfig")}`,
        content: html,
        buttons: {
          use: {
  ...
  static _getSpellData(actorData, itemData, data) {
    ...
    const consumeSpellSlot = (lvl > 0) && CONFIG.MOJO.spellUpcastModes.includes(itemData.preparation.mode);
    ...
    const spellLevels = Array.fromRange(10).reduce((arr, i) => {
      if ( i < lvl ) return arr;
      const label = CONFIG.MOJO.spellLevels[i];
      ...
      arr.push({
        level: i,
        label: i > 0 ? game.i18n.format("MOJO.SpellLevelSlot", {level: label, n: slots}) : label,
        canCast: max > 0,
        hasSlots: slots > 0
      });
      return arr;
    ...
    if (pact.level >= lvl) {
      spellLevels.push({
        level: "pact",
        label: `${game.i18n.format("MOJO.SpellLevelPact", {level: pact.level, n: pact.value})}`,
        canCast: true,
        hasSlots: pact.value > 0
      });
    }
    const canCast = spellLevels.some(l => l.hasSlots);
    if ( !canCast ) data.errors.push(game.i18n.format("MOJO.SpellCastNoSlots", {
      level: CONFIG.MOJO.spellLevels[lvl],
      name: data.item.name
    }));
  ...
  static _getAbilityUseNote(item, uses, recharge) {
    ...
    if ( quantity <= 0 ) return game.i18n.localize("MOJO.AbilityUseUnavailableHint");

    // Abilities which use Recharge
    if ( recharge.value ) {
      return game.i18n.format(recharge.charged ? "MOJO.AbilityUseChargedHint" : "MOJO.AbilityUseRechargeHint", {
    ...
    if ( item.type === "consumable" ) {
      let str = "MOJO.AbilityUseNormalHint";
      if ( uses.value > 1 ) str = "MOJO.AbilityUseConsumableChargeHint";
      else if ( item.system.quantity === 1 && uses.autoDestroy ) str = "MOJO.AbilityUseConsumableDestroyHint";
      else if ( item.system.quantity > 1 ) str = "MOJO.AbilityUseConsumableQuantityHint";
      return game.i18n.format(str, {
        type: game.i18n.localize(`MOJO.Consumable${item.system.consumableType.capitalize()}`),
        value: uses.value,
        quantity: item.system.quantity,
        max: uses.max,
        per: CONFIG.MOJO.limitedUsePeriods[uses.per]
      });
    }

    // Other Items
    else {
      return game.i18n.format("MOJO.AbilityUseNormalHint", {
        type: game.i18n.localize(`ITEM.Type${item.type.capitalize()}`),
        value: uses.value,
        max: uses.max,
        per: CONFIG.MOJO.limitedUsePeriods[uses.per]
