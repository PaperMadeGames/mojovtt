export default class SRDCompendium extends Compendium {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      ...
      template: "systems/mojo/templates/journal/srd-compendium.hbs",
      width: 800,
      height: 950,
      resizable: true
    });
  ...
  async getData(options) {
    ...
    const getOrder = o => ({chapter: 0, appendix: 100}[o.flags?.mojo?.type] ?? 200) + (o.flags?.mojo?.position ?? 0);
    ...
    data.chapters = documents.reduce((arr, entry) => {
      const type = entry.getFlag("mojo", "type");
      if ( !type ) return arr;
