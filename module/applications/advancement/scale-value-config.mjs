...
export default class ScaleValueConfig extends AdvancementConfig {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["mojo", "advancement", "scale-value", "two-column"],
      template: "systems/mojo/templates/advancement/scale-value-config.hbs",
      width: 540
    });
  }
  ...
  getData() {
    ...
    return foundry.utils.mergeObject(super.getData(), {
      ...
      movementUnits: CONFIG.MOJO.movementUnits
    });
  }
  ...
  _prepareLevelData() {
    let lastValue = null;
    return Array.fromRange(CONFIG.MOJO.maxLevel + 1).slice(1).reduce((obj, level) => {
  ...
  _onIdentifierHintCopy(event) {
    ...
    game.tooltip.activate(event.target, {text: game.i18n.localize("MOJO.IdentifierCopied"), direction: "UP"});
  }
  ...
  async _updateObject(event, formData) {
    ...
    if ( typeChange && (updates.configuration.type !== this.advancement.configuration.type) ) {
      ...
      await this.advancement.update(Array.fromRange(CONFIG.MOJO.maxLevel, 1).reduce((obj, lvl) => {
