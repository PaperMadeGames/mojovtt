...
export default class AdvancementConfig extends FormApplication {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["mojo", "advancement", "dialog"],
      template: "systems/mojo/templates/advancement/advancement-config.hbs",
      width: 400,
      height: "auto",
      submitOnChange: true,
      closeOnSubmit: false,
  ...
  get title() {
    ...
    return `${game.i18n.format("MOJO.AdvancementConfigureTitle", { item: this.item.name })}: ${type}`;
  }
  ...
  /** @inheritdoc */
  getData() {
    const levels = Object.fromEntries(Array.fromRange(CONFIG.MOJO.maxLevel + 1).map(l => [l, l]));
    if ( ["class", "subclass"].includes(this.item.type) ) delete levels[0];
    else levels[0] = game.i18n.localize("MOJO.AdvancementLevelAnyHeader");
    const context = {
      CONFIG: CONFIG.MOJO,
  ...
  async _onDrop(event) {
    ...
    if ( item.uuid === this.item.uuid ) {
      return ui.notifications.error(game.i18n.localize("MOJO.AdvancementItemGrantRecursiveWarning"));
    }

    // Abort if this uuid exists already
    if ( existingItems.includes(item.uuid) ) {
      return ui.notifications.warn(game.i18n.localize("MOJO.AdvancementItemGrantDuplicateWarning"));
    }
