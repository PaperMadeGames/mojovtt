...
export default class TraitsField {
  ...
  static get common() {
    return {
      size: new foundry.data.fields.StringField({required: true, initial: "med", label: "MOJO.Size"}),
      di: this.makeDamageTrait({label: "MOJO.DamImm"}),
      dr: this.makeDamageTrait({label: "MOJO.DamRes"}),
      dv: this.makeDamageTrait({label: "MOJO.DamVuln"}),
      ci: this.makeSimpleTrait({label: "MOJO.ConImm"})
    };
  }
  ...
  static get creature() {
    return {
      languages: this.makeSimpleTrait({label: "MOJO.Languages"})
    };
  }
  ...
  static makeSimpleTrait(schemaOptions={}, {initial=[], extraFields={}}={}) {
    return new foundry.data.fields.SchemaField({
      ...
      value: new foundry.data.fields.SetField(
        new foundry.data.fields.StringField(), {label: "MOJO.TraitsChosen", initial}
      ),
      custom: new foundry.data.fields.StringField({required: true, label: "MOJO.Special"})
    }, schemaOptions);
  }
  ...
  static makeDamageTrait(schemaOptions={}, {initial=[], initialBypasses=[], extraFields={}}={}) {
    return this.makeSimpleTrait(schemaOptions, {initial, extraFields: {
      ...
      bypasses: new foundry.data.fields.SetField(new foundry.data.fields.StringField(), {
        label: "MOJO.DamagePhysicalBypass", hint: "MOJO.DamagePhysicalBypassHint", initial: initialBypasses
      })
    }});
  }
}
