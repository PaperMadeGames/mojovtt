 ...
 * @property {string} actionType          Action type as defined in `MOJO.itemActionTypes`.
  ...
  static defineSchema() {
    return {
      ability: new foundry.data.fields.StringField({
        required: true, nullable: true, initial: null, label: "MOJO.AbilityModifier"
      }),
      actionType: new foundry.data.fields.StringField({
        required: true, nullable: true, initial: null, label: "MOJO.ItemActionType"
      }),
      attackBonus: new FormulaField({required: true, label: "MOJO.ItemAttackBonus"}),
      ...
      critical: new foundry.data.fields.SchemaField({
        threshold: new foundry.data.fields.NumberField({
          required: true, integer: true, initial: null, positive: true, label: "MOJO.ItemCritThreshold"
        }),
        damage: new FormulaField({required: true, label: "MOJO.ItemCritExtraDamage"})
      }),
      damage: new foundry.data.fields.SchemaField({
        ...
        versatile: new FormulaField({required: true, label: "MOJO.VersatileDamage"})
      }, {label: "MOJO.Damage"}),
      formula: new FormulaField({required: true, label: "MOJO.OtherFormula"}),
      save: new foundry.data.fields.SchemaField({
        ability: new foundry.data.fields.StringField({required: true, blank: true, label: "MOJO.Ability"}),
        dc: new foundry.data.fields.NumberField({
          required: true, min: 0, integer: true, label: "MOJO.AbbreviationDC"
        }),
        scaling: new foundry.data.fields.StringField({
          required: true, blank: false, initial: "spell", label: "MOJO.ScalingFormula"
        })
      }, {label: "MOJO.SavingThrow"})
    };
  }
