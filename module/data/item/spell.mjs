 ...
 * @property {string} preparation.mode           Spell preparation mode as defined in `MOJO.spellPreparationModes`.
 ...
 * @property {string} scaling.mode               Spell scaling mode as defined in `MOJO.spellScalingModes`.
  ...
  static defineSchema() {
    return this.mergeSchema(super.defineSchema(), {
      level: new foundry.data.fields.NumberField({
        required: true, integer: true, initial: 1, min: 0, label: "MOJO.SpellLevel"
      }),
      school: new foundry.data.fields.StringField({required: true, label: "MOJO.SpellSchool"}),
      components: new MappingField(new foundry.data.fields.BooleanField(), {
        required: true, label: "MOJO.SpellComponents",
        initialKeys: [...Object.keys(CONFIG.MOJO.spellComponents), ...Object.keys(CONFIG.MOJO.spellTags)]
      }),
      materials: new foundry.data.fields.SchemaField({
        value: new foundry.data.fields.StringField({required: true, label: "MOJO.SpellMaterialsDescription"}),
        consumed: new foundry.data.fields.BooleanField({required: true, label: "MOJO.SpellMaterialsConsumed"}),
        cost: new foundry.data.fields.NumberField({
          required: true, initial: 0, min: 0, label: "MOJO.SpellMaterialsCost"
        }),
        supply: new foundry.data.fields.NumberField({
          required: true, initial: 0, min: 0, label: "MOJO.SpellMaterialsSupply"
        })
      }, {label: "MOJO.SpellMaterials"}),
      preparation: new foundry.data.fields.SchemaField({
        mode: new foundry.data.fields.StringField({
          required: true, initial: "prepared", label: "MOJO.SpellPreparationMode"
        }),
        prepared: new foundry.data.fields.BooleanField({required: true, label: "MOJO.SpellPrepared"})
      }, {label: "MOJO.SpellPreparation"}),
      scaling: new foundry.data.fields.SchemaField({
        mode: new foundry.data.fields.StringField({required: true, initial: "none", label: "MOJO.ScalingMode"}),
        formula: new FormulaField({required: true, nullable: true, initial: null, label: "MOJO.ScalingFormula"})
      }, {label: "MOJO.LevelScaling"})
    });
  }
