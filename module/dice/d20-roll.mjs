...
export default class D20Roll extends Roll {
  ...
  async toMessage(messageData={}, options={}) {
    ...
    // Add appropriate advantage mode message flavor and mojo roll flags
    messageData.flavor = messageData.flavor || this.options.flavor;
    if ( this.hasAdvantage ) messageData.flavor += ` (${game.i18n.localize("MOJO.Advantage")})`;
    else if ( this.hasDisadvantage ) messageData.flavor += ` (${game.i18n.localize("MOJO.Disadvantage")})`;

    // Add reliable talent to the d20-term flavor text if it applied
    if ( this.validD20Roll && this.options.reliableTalent ) {
      const d20 = this.dice[0];
      const isRT = d20.results.every(r => !r.active || (r.result < 10));
      const label = `(${game.i18n.localize("MOJO.FlagsReliableTalent")})`;
  ...
  async configureDialog({title, defaultRollMode, defaultAction=D20Roll.ADV_MODE.NORMAL, chooseModifier=false,
    ...
    return new Promise(resolve => {
      new Dialog({
        ...
        buttons: {
          advantage: {
            label: game.i18n.localize("MOJO.Advantage"),
            callback: html => resolve(this._onDialogSubmit(html, D20Roll.ADV_MODE.ADVANTAGE))
          },
          normal: {
            label: game.i18n.localize("MOJO.Normal"),
            callback: html => resolve(this._onDialogSubmit(html, D20Roll.ADV_MODE.NORMAL))
          },
          disadvantage: {
            label: game.i18n.localize("MOJO.Disadvantage"),
  ...
  _onDialogSubmit(html, advantageMode) {
    ...
    if ( form.ability?.value ) {
      ...
      this.options.flavor += ` (${CONFIG.MOJO.abilities[form.ability.value]})`;
    }
